...
def enum_child_windows(parent_hwnd):
    """【优化版】枚举指定父窗口的内层子窗口，仅保留同进程+CEF相关的QQ子窗口"""
    child_hwnds = []
    # 第一步：获取父窗口（QQ主窗口）的进程ID，用于过滤
    parent_pid = get_window_pid(parent_hwnd)
    if not parent_pid:
        return child_hwnds

    def callback(child_hwnd, extra):
        # 过滤条件1：子窗口必须和父窗口同进程（核心！避免跨进程无关窗口）
        child_pid = get_window_pid(child_hwnd)
        if child_pid != parent_pid:
            return True  # 跳过不同进程的子窗口
        
        # 过滤条件2：子窗口类名是CEF/QQ相关（可选，进一步精准）
        child_class = win32gui.GetClassName(child_hwnd)
        cef_related_classes = ["Chrome_WidgetWin_1", "CefBrowserWindow", "Chrome_RenderWidgetHostHWND"]
        if child_class not in cef_related_classes:
            return True  # 跳过非CEF类名的子窗口
        
        # 过滤条件3：可选，保留可见/可用的子窗口（按需）
        ...
        if win32gui.IsWindowVisible(hwnd):
            ...
            if class_name == QQ_CLASS:
                ...
                if proc_name and "QQ" in proc_name:
                    # 调用优化后的enum_child_windows，仅返回QQ同进程的内层子窗口
                    child_hwnds = enum_child_windows(hwnd)
                    ...
                    qq_windows.append({
                        ...
                        'pid': pid,
                        'is_main': title == "QQ",
                        'child_hwnds': child_hwnds  # 仅包含QQ同进程的内层子窗口
                    })
        return True
